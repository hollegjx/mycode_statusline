...
try {
  ...
  const claudeDir = path.join(homeDir, '.claude', 'uucode');
  ...
  const packageMap = {
    ...
    'win32-ia32': '@gjx1/uucode-win32-x64', // Use 64-bit for 32-bit
  };
  ...
  if (!sourcePath) {
    if (!silent) {
      ...
      console.log('The global uucode command will still work via npm');
    }
    process.exit(0);
  }
